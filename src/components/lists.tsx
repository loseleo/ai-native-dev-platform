...
export function ProjectCard({ project }: { project: Project }) {
  return (
    <Card className="p-5 transition hover:border-cyan-300 hover:shadow-md">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            {[
              ...
              <Link
                ...
                className="rounded-md border border-slate-200 px-3 py-1.5 text-xs font-semibold text-slate-600 transition hover:border-cyan-300 hover:bg-cyan-50 hover:text-cyan-900"
              >
                {item.label}
              </Link>
            ))}
        ...
        <div className="min-w-48">
          ...
          <Link
            ...
            className="mt-4 inline-flex h-9 w-full items-center justify-center rounded-md bg-slate-950 px-3 text-sm font-semibold text-white transition hover:bg-slate-800"
          >
            Open Workspace
          </Link>
        </div>
...
export function DecisionCard({ decision, canWrite = false }: { decision: Decision; canWrite?: boolean }) {
  return (
    <Card className="p-4">
...
export function TaskMiniCard({ task, canWrite = false }: { task: Task; canWrite?: boolean }) {
  return (
...
export function BugRow({ bug, canWrite = false }: { bug: Bug; canWrite?: boolean }) {
  return (
